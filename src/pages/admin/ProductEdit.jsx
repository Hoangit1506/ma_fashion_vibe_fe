...
export default function ProductEdit() {
    ...
    const [isLoadingData, setIsLoadingData] = useState(true); // State loading ban đầu
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            try {
                // 1. Tải danh mục
                const catRes = await api.get('/categories', { params: { page: 1, size: 1000 } });
                setCategories(catRes.data.result.data);

                // 2. Tải chi tiết sản phẩm (Dùng API Admin)
                const prodRes = await api.get(`/products/${id}/admin`);
                ...
                // Đổ dữ liệu Variants (Có gắn thêm cờ id để biết hàng cũ/mới)
                if (data.variants && data.variants.length > 0) {
                    setVariants(data.variants.map(v => ({
                        id: v.id, // RẤT QUAN TRỌNG: Giữ lại ID
                        sku: v.sku,
                        size: v.size,
                        color: v.color,
                        price: v.price,
    ...
    // ĐÃ SỬA: Chỉ xóa khỏi mảng hiển thị, KHÔNG GỌI API DELETE (Đợi lúc Save Backend sẽ dọn)
    ...
    const addVariantRow = () => {
        // Phân loại mới thêm sẽ KHÔNG CÓ TRƯỜNG ID
        setVariants([...variants, { sku: '', size: '', color: '', price: '', comparePrice: '', weight: '', stockQuantity: '', imageUrl: '', active: true }]);
    };

    // ĐÃ SỬA: Nếu xóa Variant cũ trên form, cũng chỉ xóa khỏi mảng hiển thị
    ...
    const handleVariantImageUpload = async (index, event) => {
        ...
        if (file.size > 10 * 1024 * 1024) return alert(`File "${file.name}" vượt quá dung lượng 10MB.`);
    ...
    // ĐÃ SỬA: Chỉ xóa khỏi mảng hiển thị
    ...
    const validateForm = () => {
        ...
        if (!productInfo.name.trim()) { tempErrors.name = "Tên sản phẩm không được để trống"; isValid = false; }
        if (!productInfo.categoryId) { tempErrors.categoryId = "Vui lòng chọn danh mục"; isValid = false; }
        ...
        if (hasVariantError) {
            tempErrors.variants = "Vui lòng điền đầy đủ (Ảnh, SKU, Size, Màu, Giá, Kho) cho tất cả phân loại hàng.";
            isValid = false;
        }
    ...
    const handleSubmit = async () => {
        ...
        // ĐÃ SỬA: Gửi kèm Variant ID lên để Backend phân biệt cũ mới
        const payload = {
            ...productInfo,
            imageUrls: imageUrls,
            variants: variants.map(v => ({
                id: v.id || null, // Nếu cũ thì có ID, mới thì null
                sku: v.sku,
                size: v.size,
                color: v.color,
        ...
        try {
            await api.put(`/products/${id}`, payload); // SỬ DỤNG PUT
            alert("Cập nhật sản phẩm thành công!");
            navigate(-1); // Quay về trang trước đó
        } catch (error) {
            setErrors({ api: error.response?.data?.message || "Có lỗi xảy ra từ máy chủ, không thể cập nhật sản phẩm" });
            window.scrollTo({ top: 0, behavior: 'smooth' });
        }
    };
    ...
    if (isLoadingData) return <Box sx={{ display: 'flex', justifyContent: 'center', mt: 10 }}><CircularProgress /></Box>;

    return (
        <Box sx={{ maxWidth: '1200px', margin: '0 auto' }}>
            <style>{`.ql-editor img { max-width: 100%; height: auto; border-radius: 8px; }`}</style>
            ...
            <Paper sx={{ p: 3, mb: 3 }}>
                ...
                <Typography variant="subtitle2" fontWeight="bold" mb={1} color="text.secondary">Mô tả chi tiết</Typography>
                <Box sx={{ mb: 5 }}>
                    <ReactQuill theme="snow" value={productInfo.description} onChange={(content) => setProductInfo({ ...productInfo, description: content })} modules={quillModules} style={{ height: '350px', marginBottom: '40px' }} />
                </Box>
            </Paper>
            ...
            <Grid container spacing={3} mb={3}>
                <Grid size={{ xs: 12, md: 8 }}>
                    <Paper sx={{ p: 3, height: '100%', border: errors.images ? '1px solid red' : 'none' }}>
                        ...
                        <Button component="label" variant="outlined" startIcon={isUploading ? <CircularProgress size={20} /> : <CloudUploadIcon />} fullWidth sx={{ height: '50px', mb: 2 }} disabled={isUploading}>
                            {isUploading ? 'Đang tải...' : 'Tải lên Ảnh / Video mới'}
                ...
                <Grid size={{ xs: 12, md: 4 }}>
                    <Paper sx={{ p: 3, height: '100%' }}>
                        <Typography variant="h6" fontWeight="bold" mb={2}>Cài đặt hiển thị (Cha)</Typography>
                        <FormControlLabel control={<Switch checked={productInfo.active} onChange={(e) => setProductInfo({ ...productInfo, active: e.target.checked })} color="primary" />} label={productInfo.active ? "Đang hiển thị Sản phẩm" : "Ẩn Sản phẩm"} />
                        <Typography variant="body2" color="text.secondary" mt={2}>*Tắt ở đây sẽ ẩn toàn bộ các phân loại hàng bên dưới.</Typography>
                    </Paper>
                </Grid>
            </Grid>
            ...
            <Paper sx={{ p: 3, mb: 3, border: errors.variants ? '1px solid red' : 'none' }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                    <Typography variant="h6" fontWeight="bold" color={errors.variants ? 'error' : 'inherit'}>Quản lý Phân loại (Biến thể)</Typography>
                    ...
                    <Table size="small">
                        <TableHead sx={{ bgcolor: '#f5f5f5' }}>
                            <TableRow>
                                ...
                                <TableCell sx={{ minWidth: 80 }} title="Sản phẩm cũ không thể sửa kho ở đây">Tồn Kho (*)</TableCell>
                        ...
                        <TableBody>
                            {variants.map((row, index) => {
                                const isOldVariant = !!row.id; // Kiểm tra xem hàng này đã có trong DB chưa

                                return (
                                    <TableRow key={index} sx={{ opacity: row.active ? 1 : 0.6, bgcolor: row.active ? 'inherit' : '#fafafa' }}>
                                        <TableCell align="center">
                                            {row.imageUrl ? (
                                                <Box sx={{ position: 'relative', width: 45, height: 45, margin: '0 auto' }}>
                                                    ...
                                                    <IconButton size="small" sx={{ position: 'absolute', top: -8, right: -8, bgcolor: 'error.main', color: 'white', padding: '2px' }} onClick={() => removeVariantImage(index)}>
                                        ...
                                        <TableCell><TextField size="small" value={row.sku} onChange={(e) => handleVariantChange(index, 'sku', e.target.value)} error={errors.variants && !row.sku.trim()} /></TableCell>
                                        <TableCell><TextField size="small" value={row.size} onChange={(e) => handleVariantChange(index, 'size', e.target.value)} error={errors.variants && !row.size.trim()} /></TableCell>
                                        <TableCell><TextField size="small" value={row.color} onChange={(e) => handleVariantChange(index, 'color', e.target.value)} error={errors.variants && !row.color.trim()} /></TableCell>
                                        <TableCell><TextField size="small" type="number" value={row.price} onChange={(e) => handleVariantChange(index, 'price', e.target.value)} error={errors.variants && (row.price === '' || Number(row.price) < 0)} /></TableCell>
                                        <TableCell><TextField size="small" type="number" value={row.comparePrice} onChange={(e) => handleVariantChange(index, 'comparePrice', e.target.value)} /></TableCell>
                                        <TableCell sx={{ minWidth: 100 }}>
                                            {/* ĐÃ KHÓA CỨNG: Nếu là hàng cũ thì không cho sửa tồn kho ở đây */}
                                            <TextField
                                                size="small" type="number" value={row.stockQuantity}
                                                onChange={(e) => handleVariantChange(index, 'stockQuantity', e.target.value)}
                                                error={errors.variants && (row.stockQuantity === '' || Number(row.stockQuantity) < 0)}
                                                disabled={isOldVariant}
                                                title={isOldVariant ? "Không thể điều chỉnh tồn kho sản phẩm cũ tại đây" : ""}
                                            />
                                            {isOldVariant && (
                                                <Typography variant="caption" sx={{ display: 'block', mt: 0.5, color: '#e65100', fontWeight: 'medium' }}>
                                                    (Đang giữ: {row.reserved})
                                                </Typography>
                                            )}
                                        </TableCell>
                                        <TableCell align="center">
                                            {/* ĐÃ THÊM: Công tắc bật tắt riêng cho Variant */}
                                            <Switch size="small" checked={row.active} onChange={(e) => handleVariantChange(index, 'active', e.target.checked)} color="success" />
                                        </TableCell>
                                        <TableCell align="center">
            ...
            <Box sx={{ mt: 2, mb: 8, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                <Button variant="outlined" color="inherit" size="large" onClick={() => navigate(-1)}>HỦY</Button>
